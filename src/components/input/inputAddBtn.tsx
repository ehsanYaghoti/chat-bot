...
export default function InputAddBtn() {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="bg-transparent text-textClr-1 flex items-center justify-center hover:bg-[#454545] rounded-full w-10 h-10 cursor-pointer ">
        <Plus />
      </DropdownMenuTrigger>
      <DropdownMenuContent
        align="start"
        className="bg-netural-1 text-textClr-1 p-2 rounded-3xl border-none w-64 "
      >
        <DropdownMenuItem
        ...
        <DropdownMenuCheckboxItem
          checked={showCreateImage}
          onCheckedChange={() => setShowCreateImage(!showCreateImage)}
          className="focus:bg-token-hover focus:text-inherit hover:bg-token-hover
            rounded-lg  py-2 px-3 cursor-pointer "
          textValue="Create images"
        >
          <Images className="text-inherit" />
          Create images
        </DropdownMenuCheckboxItem>
        <DropdownMenuCheckboxItem
          checked={showCreateImage}
          ...
          textValue="Create images"
        >
          <Lightbulb className="text-inherit" />
          Thinking
        </DropdownMenuCheckboxItem>
        <DropdownMenuCheckboxItem
          checked={showCreateImage}
          ...
          textValue="Create images"
        >
          <Telescope className="text-inherit" />
          Deep research
        </DropdownMenuCheckboxItem>
        <DropdownMenuCheckboxItem
          checked={showCreateImage}
        ...
        <DropdownMenuSub>
          <DropdownMenuSubTrigger
            className="focus:bg-token-hover! focus:text-inherit! hover:bg-token-hover data-[state=open]:bg-token-hover! data-[state=open]:text-inherit!
            rounded-lg  py-2 px-3 cursor-pointer "
          >
            <Ellipsis className="text-inherit" />
            more
          </DropdownMenuSubTrigger>
          <DropdownMenuPortal>
            <DropdownMenuSubContent
              sideOffset={5}
              className={cn(
                "bg-[#353535] p-3 flex flex-col gap-1 mb-20 text-textClr-1 border-none *:hover:bg-[#4A4A4A]! *:hover:text-inherit! *:cursor-pointer rounded-3xl "
              )}
            >
              <DropdownMenuItem className="gap-2">
                <BookOpen className="text-inherit" />
                Study and learn
              </DropdownMenuItem>
              <DropdownMenuItem>
                <Globe className="text-inherit" />
                Web search
              </DropdownMenuItem>
              <DropdownMenuItem>
